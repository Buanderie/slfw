...
func main() {
	...
	requiredInboundMaps := []string{"inbound_rules", "inbound_default_policy"}
	for _, mapName := range requiredInboundMaps {
	...
	requiredInboundProgs := []string{"xdp_firewall_inbound"}
	for _, progName := range requiredInboundProgs {
	...
	// Vérifier les maps obligatoires (seulement inbound pour l'instant)
	requiredOutboundMaps := []string{"outbound_rules", "outbound_default_policy"}
	for _, mapName := range requiredOutboundMaps {
		if spec.Maps[mapName] == nil {
			fmt.Fprintf(os.Stderr, "Error: required map '%s' not found in eBPF spec\n", mapName)
			os.Exit(1)
		}
	...
	// Vérifier les programmes obligatoires (seulement inbound pour l'instant)
	requiredOutboundProgs := []string{"tc_firewall_outbound"}
	for _, progName := range requiredOutboundProgs {
		if spec.Programs[progName] == nil {
			fmt.Fprintf(os.Stderr, "Error: required program '%s' not found in eBPF spec\n", progName)
			os.Exit(1)
		}
	...
	for _, iface := range config.Firewall.Interfaces {
		...
		fmt.Printf("support: %t and map %t\n", hasOutboundSupport, outboundMap != nil)
		// Populate outbound rules seulement si supporté
		if hasOutboundSupport && outboundMap != nil {
			for i, rule := range iface.Outbound.Rules {
